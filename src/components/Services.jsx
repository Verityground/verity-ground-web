  ...
  return (
    <section id="services" className="py-24 md:py-32 bg-transparent border-b border-zinc-800/40 relative">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {/* Services Grid (Clean Modern Cards, Typography-First & Uniform Height) */}
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 items-stretch">
          {(data.services || []).map((service) => {
            const Icon = iconMap[service.icon] || Code2;
            const waCustomMsg = `Halo ${data.name || 'Verity Ground'}, saya ingin konsultasi mengenai layanan ${service.title}.`;
            const waServiceLink = `https://wa.me/${data.whatsappNumber || '6285693131477'}?text=${encodeURIComponent(waCustomMsg)}`;

            // Safe parsing for features and techStack to prevent runtime map crashes
            const safeFeatures = Array.isArray(service.features)
              ? service.features
              : (typeof service.features === 'string'
                  ? service.features.split('\n').map((f) => f.trim()).filter(Boolean)
                  : []);

            const safeTechStack = Array.isArray(service.techStack)
              ? service.techStack
              : (typeof service.techStack === 'string'
                  ? service.techStack.split(',').map((t) => t.trim()).filter(Boolean)
                  : []);

            return (
              <div
                key={service.id}
                className="p-8 rounded-xl border border-zinc-800/50 hover:border-zinc-700/80 bg-zinc-900/15 transition-all duration-200 flex flex-col justify-between h-full group"
              >
                <div>
                ...
                <div className="pt-6 border-t border-zinc-800/40 flex flex-col sm:flex-row sm:items-center justify-between gap-4 mt-auto">
                  ...
                  <a
                    ...
                    aria-label={`Konsultasi cakupan layanan ${service.title} via WhatsApp`}
                    className="inline-flex items-center gap-1.5 text-xs font-medium text-zinc-400 hover:text-white transition-colors shrink-0 group/link min-h-[44px] min-w-[44px]"
                  >
